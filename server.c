        ...
        if(sqlite3_step(stmt) == SQLITE_ROW)
        {
            ...
            if(count == 0)
            {
                const char *insert_user =
                    ...
                    "('robbob@gmail.com', 'Robby', 'Bobby', 'Rob_bob', 'password123', 100.0),"
                    "('jsmith@yahoo.com', 'John', 'Smith', 'john_sm', 'john123', 100.0),"
                    "('ajohnson@yahoo.com', 'Alice', 'Johnson', 'alice_jh', 'alice432', 100.0),"
                    "('charlieb@gmail.com', 'Charlie', 'Brown', 'charlie_b', 'brown_21', 100.0),"
                    "('dprince@yahoo.com', 'Diana', 'Prince', 'wonder_d', 'diana891', 100.0);";
                ...
                printf("Created 5 default users");
            }
        }
    }
    return 0;
...
void handle_buy(int socket, char *stock_symbol, double stock_amount, double price_per_stock, int user_id, char *response)
{
    ...
    const char *check_balance =
        "SELECT usd_balance, first_name, last_name FROM Users WHERE ID = ?;";
